...
return new class extends Migration
{
    ...
    {
        $this->normalizeLegacyPriority('supply_requests');
        $this->normalizeLegacyPriority('procurement_requests');

        Schema::table('purchase_orders', function (Blueprint $table) {
            if (! Schema::hasColumn('purchase_orders', 'priority')) {
                $table->string('priority', 32)->default('NORMAL')->index()->after('procurement_reason');
            }
            if (! Schema::hasColumn('purchase_orders', 'confirm_by')) {
                $table->date('confirm_by')->nullable()->after('delivery_date');
            }
        });

        $this->copyPriorityOntoPurchaseOrders();
        $this->updateColumnDefaults();
        $this->updatePostgresTrigger();
    }
    ...
    private function normalizeLegacyPriority(string $table): void
    {
        DB::table($table)
            ->where(function ($query) {
                $query->whereNull('priority')
                    ->orWhere('priority', '')
                    ->orWhereRaw('UPPER(priority) IN (?, ?)', ['MEDIUM', 'LOW']);
            })
            ->update(['priority' => 'NORMAL']);
    }

    private function copyPriorityOntoPurchaseOrders(): void
    {
        $orders = DB::table('purchase_orders as po')
            ->leftJoin('procurement_requests as pr', 'pr.id', '=', 'po.procurement_request_id')
            ->select('po.id', 'pr.priority')
            ->get();

        foreach ($orders as $order) {
            $priority = strtoupper(trim((string) $order->priority));
            if (! in_array($priority, ['URGENT', 'HIGH'], true)) {
                $priority = 'NORMAL';
            }

            DB::table('purchase_orders')->where('id', $order->id)->update(['priority' => $priority]);
        }
    }

    private function updateColumnDefaults(): void
    {
        $driver = Schema::getConnection()->getDriverName();

        if ($driver === 'pgsql') {
            DB::statement("ALTER TABLE supply_requests ALTER COLUMN priority SET DEFAULT 'NORMAL'");
            DB::statement("ALTER TABLE procurement_requests ALTER COLUMN priority SET DEFAULT 'NORMAL'");
        }

        if ($driver === 'mysql') {
            DB::statement("ALTER TABLE supply_requests MODIFY priority VARCHAR(32) NOT NULL DEFAULT 'NORMAL'");
            DB::statement("ALTER TABLE procurement_requests MODIFY priority VARCHAR(32) NOT NULL DEFAULT 'NORMAL'");
        }
    }

    private function updatePostgresTrigger(): void
    {
...
BEGIN
    ...
    IF NEW.status IS NULL OR BTRIM(NEW.status) = '' OR NEW.status = 'Received' THEN
        NEW.status := 'Pending';
    END IF;
    IF NEW.priority IS NULL OR BTRIM(NEW.priority) = '' OR UPPER(BTRIM(NEW.priority)) IN ('MEDIUM', 'LOW') THEN
        NEW.priority := 'NORMAL';
    ELSE
